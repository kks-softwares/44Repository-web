...
export default function ProfileotherDetails({ other, width }) {
  const [month1, setMonth1] = useState(other?.fromMonth);
  const [year1, setyear1] = useState(other?.fromYear);
  ...
  const [month2, setMonth2] = useState(other?.toMonth);
  const [year2, setyear2] = useState(other?.toYear);
  ...
  const handleeducationdelete = () => {
    ...
    axios.post(`${API_HOST}/users/removeOthers`, data).then((res) => {
      axios
  ...
  const handleeditcallage = () => {
    ...
    axios.post(`${API_HOST}/users/editOthers`, formdata).then((res) => {
      window.localStorage.setItem(
        "user",
        JSON.stringify({ ...res.data.success.data })
      );
    ...
    <>
      ...
      >
        ...
        >
            <Box sx={width > 700 ? style:style1}>
            <div className="profiletitleandmenunav">
              <div className="profiledetailstitle"> Delete Other Experince</div>
              <div className="profiledetailnavmanu">
                <div>
                  <CloseIcon
                    onClick={handleClose}
