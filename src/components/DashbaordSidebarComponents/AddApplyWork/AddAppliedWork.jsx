...
export default function AddAppliedWork({ width }) {
  ...
  useEffect(() => {
    if (user?.userName) {
      axios
        .get(
          `${API_HOST}/biding/findJobs?userName=${
            user?.userName
  ...
  return (
    <div className="BrowseWorkMain-cntainer">
      ...
      <div
        style={{
          marginBottom: width > 700 ? "0vw" : "1vw",
          marginTop: width > 700 ? "2vw" : "8vw",
        }}
      ...
      <div className="catalogcontainerdashbaord">
        {allusers?.length > 0 ? (
          allusers?.map((data) => {
            return <Listofjobbox width={width} data={data} />;
          })
        ) : (
          <div
            style={{
      ...
      {/* <div style={{marginBottom:"0vw"}} className="dashboardtitilemain">Recommended Jobs</div>
      <div className="catalogcontainerdashbaord">
      {allusers?.length > 0 &&
        allusers?.map(() => {
          return <Listofjobbox  width={width}/>;
        })}
        </div> */}
    </div>
  );
